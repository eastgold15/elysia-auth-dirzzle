...
if (typeof elysiaAuthDrizzlePlugin !== 'function') {
    throw new Error('❌ ESM Node.js failed - elysiaAuthDrizzlePlugin is not a function')
}

// 验证插件的基本结构
if (!elysiaAuthDrizzlePlugin.name || elysiaAuthDrizzlePlugin.name !== 'elysiaAuthDrizzlePlugin') {
    console.warn('⚠️ Plugin name might not be set correctly')
}

console.log('✅ ESM Node.js works! elysia-auth-drizzle plugin imported successfully')
