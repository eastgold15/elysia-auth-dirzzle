			...
			select: () => ({
				from: () => ({
					where: () => ({
						limit: () =>
							Promise.resolve([
								{ ownerId: "user123", accessToken: "valid-token" },
							]),
					}),
				}),
			}),
...
describe("Elysia Auth Drizzle Plugin", () => {
	...
	describe("URL and Method Validation", () => {
		...
		it("should handle empty config array", () => {
			const isAllowed = currentUrlAndMethodIsAllowed("/any/path", "GET", []);
			expect(isAllowed).toBe(false);
		});
	});
	...
	describe("Plugin Integration", () => {
		...
		it("should handle public routes without authentication", async () => {
			const app = new Elysia().get("/public", () => "Public content");
		...
		it("should return 404 for non-existent routes", async () => {
			const app = new Elysia().get("/existing", () => "Content");
		...
		it("should handle POST requests", async () => {
			const app = new Elysia().post("/api/data", ({ body }) => ({
				received: body,
			}));
