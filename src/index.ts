...
import { currentUrlAndMethodIsAllowed, type UrlConfig } from "./authGuard.js";
import {
	checkTokenValidity,
...
>(ORMOptions: {
	...
	const userValidation = ORMOptions?.userValidation || undefined;
	...
	return new Elysia({ name: "elysia-auth-drizzle" }).derive(
		...
		async ({ headers, query, cookie, request }) => {
			...
			let connectedUser: InferSelectModel<TUserSchema> | undefined;
